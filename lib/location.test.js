...
describe('distanceKm', () => {
  ...
  it('matches one degree of latitude for R=6371 (anchor)', () => {
...
describe('branchesByDistance', () => {
  const branches = [
    ...
    { id: 'arjan', lat: 25.068, lng: 55.239 },
  ];
  ...
  it('sorts nearest-first and annotates distanceKm', () => {
    ...
    expect(sorted.map((b) => b.id)).toEqual(['ibn-battuta', 'arjan', 'al-safa']);
    expect(sorted[0].distanceKm).toBeLessThan(sorted[1].distanceKm);
    expect(sorted[1].distanceKm).toBeLessThan(sorted[2].distanceKm);
  });
  it('picks Arjan for a customer near Dubai Miracle Garden', () => {
    const sorted = branchesByDistance(branches, 25.062, 55.244);
    expect(sorted[0].id).toBe('arjan');
  ...
  it('returns branches unsorted and unannotated for invalid coords', () => {
    for (const [lat, lng] of [[NaN, 55], [0, 0], [91, 55], [25, 181]]) {
      ...
      expect(out.map((b) => b.id)).toEqual(['al-safa', 'ibn-battuta', 'arjan']);
      expect(out[0].distanceKm).toBeUndefined();
    }
  });
  it('puts branches with invalid coords last, unannotated, and keeps them in the result', () => {
    const mixed = [
      { id: 'no-coords' },
      { id: 'al-safa', lat: 25.187, lng: 55.245 },
    ];
    const out = branchesByDistance(mixed, 25.05, 55.12);
    expect(out.map((b) => b.id)).toEqual(['al-safa', 'no-coords']);
    expect(out[0].distanceKm).toEqual(expect.any(Number));
