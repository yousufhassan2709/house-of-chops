...
describe('catalog', () => {
  ...
  it('formats prices in AED', () => {
    expect(formatPrice(99)).toBe('AED 99');
  });
});
...
describe('calculateOrder', () => {
  it('totals one classic box plus delivery', () => {
    ...
    expect(r.amountFils).toBe(11400);
  });

  it('totals a mixed cart and converts to fils', () => {
    const r = calculateOrder([{ id: 'classic', qty: 2 }, { id: 'large', qty: 1 }]);
    expect(r.subtotal).toBe(317);
    expect(r.total).toBe(332);
    expect(r.amountFils).toBe(33200);
    expect(r.lineItems).toHaveLength(2);
    expect(r.lineItems[0]).toMatchObject({ id: 'classic', qty: 2, unit_price: 99, line_total: 198 });
  });
